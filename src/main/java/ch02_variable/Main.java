...
/*
  주석(coment) : Java프로그램의 경우 폴더 및 파일의 전체 코드를 컴파일링하는데,
      거기서 오류가 하나라도 발상하게 되면 전체 프로그램이 실행이 X
...
public class Main {
    public static void main(String[] args) {
        System.out.println("안녕하세요, 제 이름은 안근수 입니다");
        ...
        /*
           ...
           하드 코딩이라는 표현을 사용합니다
           하드 코딩 : 명령문에 데이터(변수에 대입되지 않은 데이터인데 이를 리터럴(Literal)을 바로 집어넣는 것으로, '지양' 되는 방식.
         ...
         */
        int scoreEnglish = 180;              // 변수 선언 및 초기화
        ...
        /*
           ...
           개발자인 저희가 하나하나 지정을 해줘야 한다는 점
           '문자'와 '문자열'은 서로 다른 개념인데,
           문자(하나짜리)를 세 번 더했을 경우에는 문자가 아니라 문자열로 인식되기
           때문에 원하는 방식으로출력되지 않습니다.

           그렇기 때문에 71번 라인에서 sout() 내부에 ""를 넣어 원하는 결과값을 얻을 수 있었음

           즉
           작은따옴표('')는 '문자'를 표시하기 위한 기호
           큰따옴표("")를 "문자열"을 표시하기 위한 기호
           라고 알아두면 된다(하지만 python이나 JavaScript의 경우 이런 구분이 없다.)
        ...
        System.out.println(123 + "1"); // 결과값 : 1231  근데 이 경우가 예외적
        ...
        String width3 = "300";
        String width4 = "400";
        System.out.println(width3 + width4); // 의 경우 300400
        // 문자열은 연산이 아니라 "나열" 됩니다.
        ...
        Integer width5 = 500;
        Integer width6 = 600;
        System.out.println(width5 + width6); // 대문자로 시작하는 Integer가 있음
        ...
        /*
            ...
            * : 곱하기
            ...
            pi 변수를 확용 하여 원의 반지름이 43인
            원의 둘레 및 넓이를 구하시오.

            원의 둘레 공식 : 지름 x pi
            원의 넓이 공식 : 반지름의 제곱 x pi
         */
        System.out.println(pi * 43 * 2); // 270.17
        System.out.println(pi * 43 * 43);  // 5808.80
        ...
        System.out.println("안녕하세요 , 제 이름은" + name + "입니다. 제 전공은 " + major + "이고, 그래서 제 예전 직업은 " + previousJop + "였습니다. 현재는 " +
                currentJob + "을 가지고 있습니다");
        // 이상의 코드에서 알 수 있는 점은 변수 + literal의 형태로도 작성이 가능하다는 점
        ...
        제 전공은 어쩌고학과이며 ,  mbti는 머머입니다. 열심히 할게요!!!
        와 같이 출력 될 수 있도록 Sout을 작성하시오
         */

        System.out.println("저는 코리아 it 아카데미 7월 국비과정을 수강하고 있는" + name10 +"입니다"+
                " 제 전공은 " + major10 + "이며" + "mbti는" + mbti10 + "열심히 할게요!!!");
        //sout 두 번 써서 개정하는 방법
        // enter 키 위에 원화 표시 있습니다. 원화표시가 한국에서만 먹히는 형태고 원래는 열슬래시만 표기되어있습니다.
        // \n -> 개행이 일어납니다.
        // \t 행 친 효과가 일어납니다.
