...
public class CentralControl {
    ...
    // 메서드 정의
     public void addDevice(Power device) {
        // Power의 자식 클래스에 해당하는 인스턴스들을
         // deviceArray라는 필드에 추가하고 싶으면
         // 그러면 CentralControlMain에서 객체를 생성 할 때
     ...
     private  int checkEmpty() { // 메서드인데 private 사용함. Main에서 굳이}
         // 몇 개나 더 전자제품을 추가할 수 있는지 알 필요가 없어서
         // deviceArray라는 배열에서 비어있는 index를 추출해주기만 하면 됨.
         // i++ 니까 가장 빨리 만나게 되는 null값을 return하게 됨.
     ...
     /*
        ...
        하지만 0과 너무 가까운 수다 보니 -100이든지, 아니면 return 값으로 나올 수 없을만한
        음수값을 지정하는 경우도 있는데
        위에 있는 abbDevice() 메서드에서 if(checkEmpty() == -89234)로 쓰고싶지 않으면
        -1 이 가장 보편적임
    ...
    /*
        현재 배열 내에 각 객체들이 들어가 있음
        객체명.getClass().getSimpleName()을 활용하면 클래스 네임만 출력된다는 것을 확인 할 수 있는데
    ...
    public void showInfo(){
        for (int i = 0 ; i < deviceArray.length ; i++) {
            ...
            System.out.println("슬롯 [" +(i + 1) +"]:" +  deviceArray[i].getClass().getSimpleName());
        }
    }
    // 배열 내부를 돌면서 각 element들의 고유 메서드들을 실행시키기

    public void perforrmSpecificMethod() {
        for(Power device: deviceArray) {
            ...
            } else { // 아직 instanceof 연산자를 통해서 조건문을 추가하지 못한 class및 추후
                     // update를 통해서 다른 기기들을 지원할 때는 } else 사이에 else if문을 추가하면 됨.
    ...
    // 이상의 수업과정ㅇ르 거쳤을때 , printer 클래스를 정의하고 Power interface를 implement하고
    // 고유 메서드로 print() -> 프린터가 인쇄를 합니다"로 출력 될 수 있또록 정의하고,
    // Main에서 printer1을 centralControl에 추가 , on / off 실행한 후에
    //print 메서드도 centralCnotrol을 통해 실행
