...
/*
    super 키워드
        ...
        2.super.메서드명();             -> . <'점' 이 있으면 부모 클래스의 메서드를 호출한다는 의미
    ...
    자식 클래스에서 완벽한 기본 생성자를 만드는 것이 불가능함.
   ...
   이상의 방식은 정보 처리기사에서는 단골 문제이기 때문에 기억할 것.
...
public class Product extends Item{
    ...
    // 이거는 부모 생성자를 호출하는건데 , Product 클래스의 AllArgsConstructor 만들고 싶다면
    ...
    public void showInfo() {
        System.out.println("제품명:" +this.getName());        //부모거라서 getter로 조회 private
        System.out.println("카테고리:" +this.getCategory());
        System.out.println("가격:" + price);          // 자식 거라서 field로 조회
        System.out.println("재고:" + stock);          // 두줄씩 코드라인의 차이가 생기는 이유에 대해 명확히 인지하는 것이 중요
